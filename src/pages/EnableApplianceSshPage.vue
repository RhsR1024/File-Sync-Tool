...
const handleExecute = async () => {
  ...
  try {
    ...
    const response = await enableApplianceSsh(ipList);
...
<template>
  <div class="flex-1 flex flex-col bg-gradient-to-br from-slate-50 to-slate-100 p-8">
    <div class="mb-8">
      <h1 class="text-4xl font-bold text-slate-900 mb-2">{{ t('tools.applianceSsh.title') }}</h1>
      <p class="text-slate-600 text-lg">{{ t('tools.applianceSsh.description') }}</p>
    </div>

    <div class="mb-8 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
      <p class="text-blue-900 text-sm leading-relaxed">
        <span class="font-semibold">{{ t('tools.applianceSsh.info') }}</span><br>
        {{ t('tools.applianceSsh.infoDetail') }}
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2 space-y-6">
        <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
          <div class="flex items-center gap-2 mb-4">
            <h3 class="text-lg font-semibold text-slate-900">{{ t('tools.applianceSsh.selectServer') }}</h3>
            <span class="text-sm text-slate-500">({{ t('tools.applianceSsh.optional') }})</span>
          </div>
          ...
          <div v-if="serverOptions.length > 0" class="space-y-2">
            <div v-for="server in serverOptions" :key="server.id" class="flex items-center gap-3">
              ...
              />
              <label :for="`appliance-ssh-server-${server.id}`" class="flex-1 text-sm text-slate-700 cursor-pointer">
                <span class="font-medium">{{ server.name }}</span>
                <span class="text-slate-500 ml-2">({{ server.host }})</span>
              </label>
            </div>
          </div>
          <div v-else class="text-sm text-slate-500">{{ t('tools.applianceSsh.noServers') }}</div>
        </div>

        <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
          <label class="block text-lg font-semibold text-slate-900 mb-4">{{ t('tools.applianceSsh.manualIp') }}</label>
          <div class="space-y-2">
            <input
              v-model="manualIp"
              type="text"
            ...
            />
            <p class="text-xs text-slate-500">{{ t('tools.applianceSsh.manualIpHint') }}</p>
          </div>
        </div>

        <div v-if="allSelectedIps.length > 0" class="bg-slate-50 border border-slate-200 rounded-lg p-4">
          <p class="text-sm font-medium text-slate-700 mb-3">{{ t('tools.applianceSsh.selectedIps', { count: allSelectedIps.length }) }}</p>
      ...
      <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm h-fit sticky top-8">
        <h3 class="text-lg font-semibold text-slate-900 mb-4">{{ t('tools.applianceSsh.results') }}</h3>

        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <span class="text-slate-600">{{ t('tools.applianceSsh.totalLabel') }}:</span>
            <span class="text-2xl font-bold text-slate-900">{{ results.length }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-slate-600">{{ t('tools.applianceSsh.successLabel') }}:</span>
            <span class="text-2xl font-bold text-green-600">{{ successCount }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-slate-600">{{ t('tools.applianceSsh.failedLabel') }}:</span>
          ...
          <div v-if="currentProgress" class="mt-6 pt-4 border-t border-slate-200">
            <div class="text-xs text-slate-500 mb-2">
              {{ t('tools.applianceSsh.progress', { current: currentProgress.current, total: currentProgress.total }) }}
            </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-slate-200 bg-slate-50">
                <th class="px-6 py-3 text-left text-sm font-semibold text-slate-700">IP {{ t('tools.applianceSsh.address') }}</th>
                <th class="px-6 py-3 text-left text-sm font-semibold text-slate-700">{{ t('tools.applianceSsh.status') }}</th>
                <th class="px-6 py-3 text-left text-sm font-semibold text-slate-700">{{ t('tools.applianceSsh.message') }}</th>
              </tr>
            </thead>
            <tbody>
