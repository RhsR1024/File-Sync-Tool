...
assert.match(deviceSimulatorTypes, /interface PlatformServerSettings extends TargetPlatformServer[\s\S]*username: string;[\s\S]*password: string;/, 'persisted server settings must carry credentials separately from the Worker server type');
...
assert.match(page, /save-platform-servers[\s\S]*deviceSimulator\.platformAdd\.saveServerInfo/, 'the dedicated save action must expose a visible busy state');
assert.match(page, /credentialsEditableWhileRunning/, 'running devices must explain that server credentials remain editable');
