...
assert.match(page, /const logLevel = ref<SimulatorLogLevel>\('info'\)/, 'run logs must default to INFO so timing diagnostics do not flood the page');
assert.match(page, /LOG_LEVEL_PRIORITY\[entry\.level\] < LOG_LEVEL_PRIORITY\[logLevel\.value\]/, 'the level selector must use minimum-level semantics so DEBUG includes normal operational logs');
assert.match(page, /level\.toUpperCase\(\)/, 'log levels must be presented using familiar uppercase labels');
...
assert.match(messages, /description: 'INFO 默认隐藏高频实况时序诊断；排查问题时选择 DEBUG/, 'the Chinese log hint must explain when timing diagnostics appear');
...
assert.match(page, /v-for="serverItem in simulator\.platformServers\.value"/, 'the server panel must render the persisted per-server connection drafts');
assert.match(page, /v-model="serverItem\.username"/, 'every UMS server must expose its own username');
assert.match(page, /v-model="serverItem\.password"/, 'every UMS server must expose its own password');
assert.match(page, /v-model="serverItem\.host"[\s\S]*:disabled="!simulator\.platformServersEditable\.value"/, 'server addresses must remain editable in the dedicated running-safe state');
assert.match(composable, /platformServersEditable = computed[\s\S]*'running'/, 'the running state must explicitly allow server draft edits');
assert.match(page, /platformPasswordVisible\(serverItem\.id\) \? 'text' : 'password'/, 'each UMS password must support explicit reveal and conceal');
assert.match(deviceSimulatorTypes, /interface PlatformServerSettings extends TargetPlatformServer[\s\S]*username: string;[\s\S]*password: string;[\s\S]*auto_register_devices: boolean;[\s\S]*replace_existing_devices: boolean;/, 'persisted server settings must carry credentials and registration policy separately from the Worker server type');
assert.match(composable, /last_platform_servers\.map\(\(\{ id, host, port \}\) => \(\{ id, host, port \}\)\)/, 'UMS credentials must be stripped when settings become a Worker start request');
assert.match(platformRegistration, /&server\.username[\s\S]*&server\.password/, 'registration must use each server’s saved credentials');
assert.match(page, /savePlatformServerInfo[\s\S]*simulator\.savePlatformServers\(\)/, 'server information must have a dedicated save action');
assert.match(page, /save-platform-servers[\s\S]*deviceSimulator\.platformAdd\.saveAndApply/, 'the dedicated apply action must expose a visible busy state');
assert.match(page, /serversEditableWhileRunning/, 'running devices must explain that server configuration can be applied live');
...
assert.match(page, /v-model="serverItem\.replace_existing_devices"/, 'every server row must expose the replace-existing preference');
assert.match(messages, /replaceBeforeRegister: '添加前删除同IP设备'/, 'replacement must clearly state that matching IP devices are deleted before adding');
assert.match(composable, /replace_existing_devices: false/, 'destructive replacement must be unchecked for new servers by default');
assert.match(settingsProjection, /last_platform_servers: platformServers\.value\.map/, 'per-server registration preferences must survive the settings projection');
...
assert.match(composable, /confirmPlatformReplaceRetry[\s\S]*replaceExisting: true/, 'confirming replacement must force query-delete-add mode');
