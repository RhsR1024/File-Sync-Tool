...
import { createSyncTaskNotificationTracker } from './syncTaskNotifications.ts';
...
test('reports copy and deploy milestones once for the same scheduled run', () => {
  ...
  assert.deepEqual(tracker.collect([group({ copy_status: 'completed', deploy_status: 'running' })]), [
    { kind: 'deploy_started', taskName: 'build-1' },
  ]);
  assert.deepEqual(tracker.collect([group({ copy_status: 'completed', deploy_status: 'completed' })]), [
