...
当前屏幕共享已经具备 Windows 屏幕采集、MJPEG 浏览器观看、多显示器选择、访问密码、二维码、观看人数、连接 IP、捕获异常恢复等能力，但浏览器只能被动观看。共享者的 `ScreenSharePage.vue` 目前只显示本机状态和日志，也没有订阅批注事件，因此共享者默认看不到观看者画的内容。
...
| 决策项 | 本 Spec 选择 | 理由 |
|---|---|---|
| 视频传输 | P0 保留 MJPEG，预留可插拔传输层 | 先保证兼容性，再用实测决定是否增加 H.264/MSE 或 WebRTC |
| 实时交互 | 新增同源 WebSocket `/session/ws` | 同时承载批注、会话状态和控制事件 |
| 批注呈现 | 浏览器透明 SVG/Canvas 叠层 | 不增加 JPEG 编码负担，便于撤销和同步 |
| 共享者可见性 | P0 提供应用内预览；桌面叠加列为可选增强 | 共享者可以看到同一份权威批注，又不把透明窗口强行放到桌面上 |
...
| WebRTC | P0 不做，保留为低延迟媒体候选 | 如果实测 MJPEG 带宽或延迟不达标，再增加 WebRTC |
...
### 4.3 P2：可选增强 — 共享者桌面叠加与基础键盘

- 可选创建覆盖当前共享显示器的透明、置顶、鼠标穿透窗口，显示同一份批注。
- 叠加窗口必须跟随活动显示器的 `sourceEpoch`、DPI、分辨率和显示器拓扑变化移动或隐藏。
- 叠加窗口不显示视频，不把浏览器控件带到共享桌面，也不获取键盘焦点。
- 若某个捕获后端会把叠加窗口再次捕获，必须用实机验证并避免形成反馈；P0 的应用内预览不依赖该行为。
...
- P0 不做音频、麦克风和自适应码率；媒体评估阶段之外不承诺 H.264/AV1。
...
当前实现已经是“采集一次、JPEG 编码一次、`Arc<Bytes>` 广播给多个观看者”：

```text
WGC/DXGI BGRA
  -> CPU BGRA 转 RGB
  -> image::JpegEncoder
  -> tokio::broadcast<Arc<Bytes>>
  -> 每个观看者的 multipart MJPEG
```

这条链路的优点是部署简单、浏览器兼容好、断线重连容易；缺点是没有帧间压缩。捕获后端在画面无变化时可能不产生新帧，但动态/滚动画面仍会反复传完整 JPEG，观看者越多，出口带宽越快成为瓶颈。

### 5.1.1 候选方案

| 方案 | 带宽/延迟潜力 | 改造量 | 结论 |
|---|---|---:|---|
| MJPEG 优化 | 带宽不变数量级；当前延迟基线 | 低 | P0 保留和回退 |
| WebSocket 二进制 JPEG | 与 MJPEG 基本相同 | 低-中 | 适合把帧元数据和批注放到同一控制面，不是压缩升级 |
| H.264 + MSE/fMP4 | 通常比 JPEG 低一个数量级；约 200-500ms 可调 | 中-高 | 适合单向局域网广播，要求编码器和浏览器基准 |
| WebRTC | LAN 可达到约 50-200ms，带宽自适应 | 高 | 适合远控、低延迟和未来音频；先做可选传输 |
| HLS/DASH/RTSP/裸 BGRA | 延迟高、浏览器不原生或带宽过大 | 高 | 不考虑 |

数值只是工程目标和量级，不是承诺；最终以 Windows 实机在静态、滚动和视频三种负载下测量为准。

### 5.1.2 推荐媒体路线

1. **P0：MJPEG 基线不删除**。先将视频发送抽象成 `MediaTransport`，批注和控制只依赖 `session_id`、`source_epoch` 和 `frame_id`，不依赖 JPEG 细节。
2. **P0：做实际基准**。分别测试 1、5、10 个观看者的 CPU、出口 Mbps、端到端延迟、实际 FPS、掉帧和重连时间。
3. **P1：若带宽或延迟不达标，增加 H.264 路径**。优先 Windows Media Foundation 或硬件编码器；浏览器使用 MSE/fMP4 或 WebRTC 播放，MJPEG 作为兼容回退。MSE 必须处理 init segment、周期 IDR、append 队列、重连和 `sourceEpoch` 变化时的 flush。
4. **P1/P2：如果进入远控，优先 WebRTC**。视频轨道提供低延迟反馈，DataChannel 承载鼠标、键盘和协作事件；局域网首版可以不配置 STUN/TURN，但每个 peer 仍需要独立的 RTP/RTCP 状态，不能简单复制同一串 RTP 字节。
5. **不在没有基准数据时直接删除 MJPEG**。旧浏览器、故障恢复和诊断回退仍需要一个简单路径。

基准应在交互协议中携带 `frameId` 和采集时间戳；不能只依赖 MJPEG `<img>` 的 `onload`，因为多数浏览器只在首帧触发该事件。现有 `bytes_sent` 和 `fps_counter` 可作为共享端的初始统计来源。

### 5.1.3 MJPEG 可先做的优化

- 根据观看人数和实际出口带宽动态降低 FPS、质量或分辨率。
- 保持单次编码和共享字节，避免每个观看者重复编码。
- 评估 libjpeg-turbo 或 Windows 硬件编码，避免继续增加 CPU 转色/拷贝。
- 继续使用“慢客户端跳到最新帧”，但不要让该策略用于批注和控制消息。
- 记录每个会话的实际 JPEG 大小、出口带宽和帧年龄，为媒体切换提供数据。

### 5.1.4 统一播放契约

不论最终使用 MJPEG、MSE 还是 WebRTC，服务端都应向交互层提供：
...
这样批注层只负责在正确的画面坐标上绘制，不需要随着视频编码器重写。
...
首期采用“同一份批注状态，多种渲染端”的方式：

- 所有通过观看页连接的浏览器都能看到。
- 共享者在桌面应用的本机预览中看到同一画面、冻结状态和批注。
- 直接请求 `/stream` 的第三方 MJPEG 客户端不会看到矢量批注。
- 共享者物理桌面上的透明叠加窗口属于 P2，不阻塞 P0/P1。

本机预览可以复用独立观看端作为一个受控的本地客户端，或由 Tauri 页面订阅同一 WebSocket 并在预览图像上绘制 SVG。推荐前者，确保共享者看到的效果与观看者一致；它会额外占用一个本地观看连接，但不需要改变捕获线程。

如果未来必须让所有直接消费视频的客户端也看到批注，再增加“带批注视频”输出，在编码前合成矢量层。该模式不作为默认路径。

### 6.3 共享者视图实现选择

按实现成本和用户体验分三档：

| 方式 | 共享者看到什么 | 适合阶段 | 主要代价 |
|---|---|---|---|
| 本机预览客户端 | 与浏览器观看者完全一致的画面、冻结和批注 | P0，推荐 | 多一个本地视频连接和 WebSocket 客户端 |
| Tauri 页面直接绘制 | Tauri 页面自己的预览图像和批注层 | P1 | 需要处理 Tauri origin、认证和画面尺寸同步 |
| Windows 桌面叠加 | 批注直接出现在真实共享桌面上 | P2 | 透明置顶窗口、DPI/多屏、捕获反馈和焦点处理 |

P0 采用本机预览客户端。它不依赖 WGC/DXGI 是否会捕获透明窗口，也不会把批注窗口本身变成视频内容。若共享者只想看状态而不打开预览，仍可关闭本机预览，不影响远端观看。

若共享启用了密码，本机预览不能假定 Tauri WebView 自动拥有浏览器的 `ss_auth` Cookie。实现应由 Rust 为本机预览注入一次性本地会话/能力，或提供只绑定 `127.0.0.1` 的 host-preview 路由；不得把共享密码直接拼进预览 URL。

P2 桌面叠加可以复用项目已有的透明、无边框、置顶 Tauri WebviewWindow 模式，窗口只渲染 SVG/Canvas 批注，不加载 `/stream` 背景，并设置鼠标穿透和不抢焦点。Windows 后端应尝试 `WDA_EXCLUDEFROMCAPTURE`，避免叠加层被 DXGI/WGC 再捕获形成双层或反馈；两个捕获后端都必须实机验证。若排除捕获不可用，则回退到 P0 应用内预览，不把不确定的桌面叠加作为默认行为。
...
type ScreenShareMediaTransport = 'auto' | 'mjpeg' | 'mse_h264' | 'webrtc';

// P0 的 auto 解析为 MJPEG；其他值只有对应传输实现和浏览器能力都可用时才生效。
interface ScreenShareMediaConfig {
  transport: ScreenShareMediaTransport;
}
```

字段同步进入 Rust、`src/lib/tauri.ts`、页面保存设置和默认值。旧设置缺字段时必须使用上述默认值。媒体传输选择不能改变交互 WebSocket 的消息契约。
...
| `screen_share_open_local_preview` | 无 | `Result<void, String>` | 打开或聚焦共享者本机预览 |
| `screen_share_close_local_preview` | 无 | `Result<void, String>` | 关闭本机预览，不停止共享 |
...
| `screen-share-annotation-state` | `AnnotationDocument` 或增量事件 | 供本机预览/可选桌面叠加渲染 |
...
| `GET` | `/host-preview` | 仅回环地址可用的本机预览入口，使用 Rust 注入的一次性本地能力 |
...
- “打开本机预览”按钮；预览关闭不影响远端观看。
- 媒体传输选择显示为“自动（推荐）/MJPEG/实验性 H.264 或 WebRTC”，实验性选项只在实现和浏览器探测通过时展示。
...
- 当前视频传输、实际 FPS、出口带宽和最近一帧年龄，供媒体方案实测比较。
...
7. 共享者打开本机预览后，能看到与浏览器相同的实时画面、冻结状态和批注；关闭预览不影响远端观看。
8. 10 个浏览器同时连接并进行低频批注时，共享端无明显卡顿，视频 FPS 不因静止批注下降超过 10%。
...
7. 从浏览器操作到下一帧看到结果的局域网 P95 目标不超过 300ms；该指标不绑定某一种视频传输实现。

### 14.3 媒体评估

1. 以 1、5、10 个观看者分别测试静态桌面、连续滚动和视频播放。
2. 记录共享端 CPU、内存、出口 Mbps、实际 FPS、帧年龄、首帧时间和重连时间。
3. MJPEG 基线必须可用；H.264/MSE 或 WebRTC 只有在带宽或延迟实测明显改善且浏览器覆盖满足要求时才进入实现。
4. 传输切换不能改变批注坐标、冻结帧、`frameId`、`sourceEpoch` 和 WebSocket 消息契约。
...
- 抽象 `MediaTransport`，但默认仍使用 MJPEG。
...
- 完成 1/5/10 观看者的 MJPEG 基准。
- 若 MJPEG 成为带宽或延迟瓶颈，再实现 H.264/MSE 或 WebRTC 的实验性路径。
- 保留 MJPEG 回退，不让媒体实验阻塞批注功能。
...
1. **共享者如何看到批注**：本 Spec 的 P0 是应用内预览；如果“看到”特指正在操作的真实桌面，则必须把透明桌面叠加从 P2 提升到 P0/P1，不能把两者视为同一体验。
...
7. **视频技术**：本 Spec 选择 MJPEG 作为兼容基线，并允许用实测触发 H.264/MSE 或 WebRTC；音频仍不做。
