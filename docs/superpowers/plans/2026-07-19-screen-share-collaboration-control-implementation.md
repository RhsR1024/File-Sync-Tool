...
键盘控制保持独立开关、默认关闭。共享者批准控制申请后，转发浏览器能够捕获的全部标准按键和系统组合键，并覆盖常见国际键盘、导航键、数字键盘、功能键、媒体键和 Windows 键；仅拒绝无法识别或没有 Windows 注入表示的键码。浏览器、观看端操作系统、Windows 安全桌面和进程完整性级别仍可能阻止部分按键实际生效。
